...
namespace B_HumanList
{
    public partial class Form1 : Form
    {
        ...
        private string StrSQL = @"Provider=Microsoft.ACE.OLEDB.16.0;Data Source=
 humaninfo.accdb;Mode=ReadWrite"; //데이터베이스 연결 문자열
        private int num = 0;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (txtCheck() == true)
            {
                if (txtCheck() == true)
                {
                    ...
                    this.txtPhone.Text + "', '" + this.txtJob.Text + "')"; var Comm = new OleDbCommand(Sql, Conn);
                    int i = Comm.ExecuteNonQuery(); Conn.Close();
                    if (i == 1)
                     {
        ...
        private void txtClear() //입력란 공백처리
        {
            this.txtName.Clear();
            this.txtAge.Clear();
            this.txtPhone.Clear();
        ...
        private void lvList_DataSet_View()
        {
            ...
            IEnumerable<DataRow> query = from HumanInfo in dt.AsEnumerable()
                                         select HumanInfo;

            foreach(DataRow HumData in query)
            {
                var strArray = new String[] {HumData.Field<int>("m_id").ToString(),
        ...
        private void btnDel_Click(object sender, EventArgs e)
        {
            if (this.lvList.SelectedItems.Count > 0)
            {
                DialogResult dlr = MessageBox.Show("선택한 데이터를 삭제할까요?", "알림",
                    MessageBoxButtons.YesNo, MessageBoxIcon.Question); switch (dlr)
                {
                    case DialogResult.Yes:
                        var Conn = new OleDbConnection(StrSQL); Conn.Open();
                        string Sql = "DELETE FROM t_info WHERE m_id =" + num + "";
                        var Comm = new OleDbCommand(Sql, Conn); int i = Comm.ExecuteNonQuery();
                        Conn.Close();
                        if (i == 1)
                        {
                            MessageBox.Show("정상적으로 데이터가 삭제되었습니다.",
                                "알림", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            lvList_DataSetRamda_View();
                            txtClear();
                        }
                        else
                        { MessageBox.Show("정상적으로 데이터가 삭제되지 않았습니다.",
                            "에러", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                        break;
